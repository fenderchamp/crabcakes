...
$player1->add_card($deck->get_card('6H'));
...
is ( $game->player_with_the_most(6),$player1,'ones gots more sixes');
...
throws_ok { $game->starting_player() } "CrabCakes::Error::AllPlayersNotReady", 'everybody ain\'t ready boss';
throws_ok { $game->starting_player() } qr/^all players must be ready, not ready 0,1/, 'exception message';
...
throws_ok { $game->starting_player() } "CrabCakes::Error::AllPlayersNotReady", 'everybody ain\'t ready boss';
throws_ok { $game->starting_player() } qr/^all players must be ready, not ready 1/, 'exception message';
