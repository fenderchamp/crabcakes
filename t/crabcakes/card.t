...
dies_ok { my $card = CrabCakes::Card->new( number => 2 ) }
'dies only number no suit';
dies_ok { my $card = CrabCakes::Card->new( number => 19, suit => 'diamonds' ) }
'dies only bad number good suit';
dies_ok { my $card = CrabCakes::Card->new( number => 2, suit => 'dime' ) }
